...
BaseFolder: TypeAlias = Literal["app", "pages", "widgets",'features', "entites", "shared"]
...
import css from "./{self._element.name}.module.css"

interface {self._element.name}Props {{
  
}}

export const {self._element.name}: FC<{self._element.name}Props> = ({{}}) => (
...
class IndexFileCreator(FileCreator):
    ...
    def _write_file_contents(self):
        ...
        if current_file_contents.strip():
            return
        self.get_absolute_filename().write_text(
            f"""export {{ {self._element.name} }} from "./{self._element.name}";"""
        )
    ...
    def _ask_base_folder(self) -> BaseFolder:
        while True:
            match input("a - app, \np - pages, \nw - widgets,\n"
                ...
                case "a": return "app"
                case "p": return "pages"
                case "w": return "widgets"
                case "f": return "features"
                case "e": return "entites"
